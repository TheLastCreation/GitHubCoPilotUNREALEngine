...
FString FGitHubCopilotUEToolExecutor::ExecuteTool(const FString& ToolName, const TSharedPtr<FJsonObject>& Arguments)
{
	...
	if (ToolName == TEXT("inspect_asset"))        return Tool_InspectAsset(Arguments);
	if (ToolName == TEXT("modify_asset"))         return Tool_ModifyAsset(Arguments);
	if (ToolName == TEXT("create_asset"))         return Tool_CreateAsset(Arguments);
...
FString FGitHubCopilotUEToolExecutor::Tool_InspectAsset(const TSharedPtr<FJsonObject>& Args)
{
	if (!AssetService.IsValid())
	{
		return TEXT("Error: Asset service is not available");
	}
	...
	bool bBlueprintDefaults = false;
	Args->TryGetBoolField(TEXT("blueprint_defaults"), bBlueprintDefaults);

	FString Result;
	if (!AssetService->InspectAsset(AssetPath, bBlueprintDefaults, Result))
	{
		return FString::Printf(TEXT("Error: %s"), *Result);
	}
...
FString FGitHubCopilotUEToolExecutor::Tool_ModifyAsset(const TSharedPtr<FJsonObject>& Args)
{
	if (!AssetService.IsValid())
	{
		return TEXT("Error: Asset service is not available");
	...
	if (!Args->TryGetArrayField(TEXT("operations"), Operations) || Operations == nullptr || Operations->Num() == 0)
	{
	...
	FString Result;
	if (!AssetService->ModifyAsset(AssetPath, bBlueprintDefaults, *Operations, Result))
	{
		return FString::Printf(TEXT("Error: %s"), *Result);
	}
...
FString FGitHubCopilotUEToolExecutor::Tool_CreateAsset(const TSharedPtr<FJsonObject>& Args)
{
	if (!AssetService.IsValid())
	{
		return TEXT("Error: Asset service is not available");
	...
	FString AssetClass;
	if (!Args->TryGetStringField(TEXT("asset_class"), AssetClass) || AssetClass.IsEmpty())
	{
		return TEXT("Error: 'asset_class' is required");
	}
	...
	Args->TryGetObjectField(TEXT("properties"), Properties);

	FString Result;
	if (!AssetService->CreateAsset(AssetClass, AssetName, PackagePath, Properties ? *Properties : nullptr, bOpenEditor, Result))
	{
		return FString::Printf(TEXT("Error: %s"), *Result);
	}
...
static TSharedPtr<FJsonObject> MakeTypedProp(const FString& Type, const FString& Desc)
{
	TSharedPtr<FJsonObject> Prop = MakeShareable(new FJsonObject);
	Prop->SetStringField(TEXT("type"), Type);
	Prop->SetStringField(TEXT("description"), Desc);
...
static TSharedPtr<FJsonObject> MakeNumberArrayProp(const FString& Desc)
{
	TSharedPtr<FJsonObject> Prop = MakeTypedProp(TEXT("array"), Desc);
	Prop->SetObjectField(TEXT("items"), MakeTypedProp(TEXT("number"), TEXT("Numeric component")));
	Prop->SetNumberField(TEXT("minItems"), 3);
	Prop->SetNumberField(TEXT("maxItems"), 3);
	return Prop;
}

static TSharedPtr<FJsonObject> MakeLooseObjectProp(const FString& Desc)
{
	TSharedPtr<FJsonObject> Prop = MakeTypedProp(TEXT("object"), Desc);
	Prop->SetBoolField(TEXT("additionalProperties"), true);
	return Prop;
}

static TSharedPtr<FJsonObject> MakeOperationItemProp()
{
	TSharedPtr<FJsonObject> Item = MakeTypedProp(TEXT("object"), TEXT("Asset modification operation"));
	TSharedPtr<FJsonObject> Props = MakeShareable(new FJsonObject);
	AddProp(Props, TEXT("type"), TEXT("string"), TEXT("Operation type: set_property, clear_property, add_array_item, remove_array_item, replace_array_item, or add_mapping for Input Mapping Context assets"));
	AddProp(Props, TEXT("property"), TEXT("string"), TEXT("Top-level editable property name for generic reflection-based operations"));
	AddProp(Props, TEXT("index"), TEXT("integer"), TEXT("Array index for remove_array_item or replace_array_item"));
	AddProp(Props, TEXT("action"), TEXT("string"), TEXT("Enhanced Input action asset path for add_mapping (for example /Game/Input/Actions/IA_CharacterScreen.IA_CharacterScreen)"));
	AddProp(Props, TEXT("key"), TEXT("string"), TEXT("Enhanced Input key name for add_mapping (for example C or Gamepad_FaceButton_Bottom)"));
	Item->SetObjectField(TEXT("properties"), Props);
	Item->SetBoolField(TEXT("additionalProperties"), true);
	return Item;
}
...
TArray<TSharedPtr<FJsonValue>> FGitHubCopilotUEToolExecutor::BuildToolDefinitions()
{
	...
	// ── spawn_actor ──
	{
		...
		Props->SetObjectField(TEXT("location"), MakeNumberArrayProp(TEXT("Spawn location [X, Y, Z] (default [0,0,0])")));
		Props->SetObjectField(TEXT("rotation"), MakeNumberArrayProp(TEXT("Spawn rotation [Pitch, Yaw, Roll] in degrees (default [0,0,0])")));
		Props->SetObjectField(TEXT("scale"), MakeNumberArrayProp(TEXT("Scale [X, Y, Z] (default [1,1,1])")));
		Params->SetObjectField(TEXT("properties"), Props);
		TArray<TSharedPtr<FJsonValue>> Req;
	...
	// ── inspect_asset ──
	{
		...
		Params->SetStringField(TEXT("type"), TEXT("object"));
		TSharedPtr<FJsonObject> Props = MakeShareable(new FJsonObject);
		AddProp(Props, TEXT("asset_path"), TEXT("string"), TEXT("Content Browser asset path such as /Game/Input/IMC_Default or /Game/Input/IMC_Default.IMC_Default"));
		AddProp(Props, TEXT("blueprint_defaults"), TEXT("boolean"), TEXT("If true, inspect the Blueprint generated class default object instead of the asset instance"));
		Params->SetObjectField(TEXT("properties"), Props);
		TArray<TSharedPtr<FJsonValue>> Req;
		Req.Add(MakeShareable(new FJsonValueString(TEXT("asset_path"))));
		Params->SetArrayField(TEXT("required"), Req);
		Params->SetBoolField(TEXT("additionalProperties"), false);
		Tools.Add(MakeToolDef(TEXT("inspect_asset"),
			TEXT("Inspect an Unreal asset or Blueprint defaults target and return editable reflected properties with current values. Use this before modify_asset to discover real property names and types."),
			Params));
	}
	...
	// ── modify_asset ──
	{
		...
		Params->SetStringField(TEXT("type"), TEXT("object"));
		TSharedPtr<FJsonObject> Props = MakeShareable(new FJsonObject);
		AddProp(Props, TEXT("asset_path"), TEXT("string"), TEXT("Content Browser asset path such as /Game/Input/IMC_Default or /Game/Blueprints/BP_Controller.BP_Controller"));
		AddProp(Props, TEXT("blueprint_defaults"), TEXT("boolean"), TEXT("If true, apply changes to the Blueprint generated class default object"));
		TSharedPtr<FJsonObject> OperationsProp = MakeShareable(new FJsonObject);
		OperationsProp->SetStringField(TEXT("type"), TEXT("array"));
		OperationsProp->SetStringField(TEXT("description"), TEXT("Ordered list of asset modification operations"));

		TSharedPtr<FJsonObject> OperationItemProp = MakeShareable(new FJsonObject);
		OperationItemProp->SetStringField(TEXT("type"), TEXT("object"));
		OperationItemProp->SetStringField(TEXT("description"), TEXT("Asset modification operation"));
		TSharedPtr<FJsonObject> OperationItemProps = MakeShareable(new FJsonObject);
		AddProp(OperationItemProps, TEXT("type"), TEXT("string"), TEXT("Operation type: set_property, clear_property, add_array_item, remove_array_item, replace_array_item, or add_mapping for Input Mapping Context assets"));
		AddProp(OperationItemProps, TEXT("property"), TEXT("string"), TEXT("Top-level editable property name for generic reflection-based operations"));
		AddProp(OperationItemProps, TEXT("index"), TEXT("integer"), TEXT("Array index for remove_array_item or replace_array_item"));
		AddProp(OperationItemProps, TEXT("action"), TEXT("string"), TEXT("Enhanced Input action asset path for add_mapping (for example /Game/Input/Actions/IA_CharacterScreen.IA_CharacterScreen)"));
		AddProp(OperationItemProps, TEXT("key"), TEXT("string"), TEXT("Enhanced Input key name for add_mapping (for example C or Gamepad_FaceButton_Bottom)"));
		OperationItemProp->SetObjectField(TEXT("properties"), OperationItemProps);
		OperationItemProp->SetBoolField(TEXT("additionalProperties"), true);
		OperationsProp->SetObjectField(TEXT("items"), OperationItemProp);
		Props->SetObjectField(TEXT("operations"), OperationsProp);
		Params->SetObjectField(TEXT("properties"), Props);
		TArray<TSharedPtr<FJsonValue>> Req;
		Req.Add(MakeShareable(new FJsonValueString(TEXT("asset_path"))));
		Req.Add(MakeShareable(new FJsonValueString(TEXT("operations"))));
		Params->SetArrayField(TEXT("required"), Req);
		Params->SetBoolField(TEXT("additionalProperties"), false);
	...
	// ── create_asset ──
	{
		...
		Params->SetStringField(TEXT("type"), TEXT("object"));
		TSharedPtr<FJsonObject> Props = MakeShareable(new FJsonObject);
		AddProp(Props, TEXT("asset_class"), TEXT("string"), TEXT("Supported asset class to create. Current phase-1 values: InputAction, InputMappingContext"));
		AddProp(Props, TEXT("asset_name"), TEXT("string"), TEXT("Name of the new asset"));
		AddProp(Props, TEXT("package_path"), TEXT("string"), TEXT("Content path package such as /Game/Input/Actions"));
		TSharedPtr<FJsonObject> CreateAssetPropertiesProp = MakeShareable(new FJsonObject);
		CreateAssetPropertiesProp->SetStringField(TEXT("type"), TEXT("object"));
		CreateAssetPropertiesProp->SetStringField(TEXT("description"), TEXT("Optional editable properties to set immediately after creation"));
		CreateAssetPropertiesProp->SetBoolField(TEXT("additionalProperties"), true);
		Props->SetObjectField(TEXT("properties"), CreateAssetPropertiesProp);
		AddProp(Props, TEXT("open_editor"), TEXT("boolean"), TEXT("Whether to open the asset editor after creation. Default: true"));
		Params->SetObjectField(TEXT("properties"), Props);
		TArray<TSharedPtr<FJsonValue>> Req;
		Req.Add(MakeShareable(new FJsonValueString(TEXT("asset_class"))));
		Req.Add(MakeShareable(new FJsonValueString(TEXT("asset_name"))));
		Params->SetArrayField(TEXT("required"), Req);
		Params->SetBoolField(TEXT("additionalProperties"), false);
		Tools.Add(MakeToolDef(TEXT("create_asset"),
			TEXT("Create a supported Unreal asset in the Content Browser. Phase 1 supports Enhanced Input assets like InputAction and InputMappingContext."),
			Params));
	}
	...
	NormalizeToolDefinitions(Tools, false);
	return Tools;
}
#endif
// Copyright GitHub, Inc. All Rights Reserved.
...
namespace
{
		...
		int32 MaxItems = 0)
	{
		...
		if (MinItems > 0)
		{
			Prop->SetNumberField(TEXT("minItems"), MinItems);
		}
		if (MaxItems > 0)
		{
			Prop->SetNumberField(TEXT("maxItems"), MaxItems);
		}

		Props->SetObjectField(Name, Prop);
	}

	static FString InferArrayItemType(const FString& SchemaName, const TSharedPtr<FJsonObject>& Schema)
	{
		const FString LowerName = SchemaName.ToLower();
		const FString LowerDesc = Schema.IsValid() && Schema->HasField(TEXT("description"))
			? Schema->GetStringField(TEXT("description")).ToLower()
			: TEXT("");

		if (LowerName == TEXT("operations"))
		{
			return TEXT("object");
		}

		if (LowerName == TEXT("location")
			|| LowerName == TEXT("rotation")
			|| LowerName == TEXT("scale")
			|| LowerName == TEXT("base_color")
			|| LowerName == TEXT("emissive_color"))
		{
			return TEXT("number");
		}

		if (LowerDesc.Contains(TEXT("[x, y, z]"))
			|| LowerDesc.Contains(TEXT("[r, g, b]"))
			|| LowerDesc.Contains(TEXT("numeric"))
			|| LowerDesc.Contains(TEXT("values 0-1"))
			|| LowerDesc.Contains(TEXT("degrees")))
		{
			return TEXT("number");
		}

		if (LowerDesc.Contains(TEXT("operation")))
		{
			return TEXT("object");
		}
	...
	static void NormalizeSchemaObject(const TSharedPtr<FJsonObject>& Schema, const FString& SchemaName)
	{
		if (!Schema.IsValid())
		{
			return;
		...
		{
			const TSharedPtr<FJsonObject> Properties = Schema->GetObjectField(TEXT("properties"));
			if (Properties.IsValid())
			{
				for (const TPair<FString, TSharedPtr<FJsonValue>>& PropPair : Properties->Values)
				{
					NormalizeSchemaObject(PropPair.Value.IsValid() ? PropPair.Value->AsObject() : nullptr, PropPair.Key);
				}
			}
		}

		const FString Type = Schema->HasField(TEXT("type")) ? Schema->GetStringField(TEXT("type")) : TEXT("");
		if (Type == TEXT("array"))
		{
			TSharedPtr<FJsonObject> Items = Schema->HasTypedField<EJson::Object>(TEXT("items"))
				? Schema->GetObjectField(TEXT("items"))
				: nullptr;

			if (!Items.IsValid() || !Items->HasField(TEXT("type")) || Items->GetStringField(TEXT("type")).TrimStartAndEnd().IsEmpty())
			{
				Items = MakeShareable(new FJsonObject);
				Items->SetStringField(TEXT("type"), InferArrayItemType(SchemaName, Schema));
				Schema->SetObjectField(TEXT("items"), Items);
			}

			NormalizeSchemaObject(Items, SchemaName + TEXT(".items"));
		}
	}

	static void NormalizeToolDefinitions(TArray<TSharedPtr<FJsonValue>>& Tools, bool bResponsesFormat)
	{
		for (TSharedPtr<FJsonValue>& ToolVal : Tools)
		{
			const TSharedPtr<FJsonObject> ToolObj = ToolVal.IsValid() ? ToolVal->AsObject() : nullptr;
			if (!ToolObj.IsValid())
			{
				continue;
			}
			...
			if (bResponsesFormat)
			{
				if (ToolObj->HasTypedField<EJson::Object>(TEXT("parameters")))
				{
					NormalizeSchemaObject(ToolObj->GetObjectField(TEXT("parameters")), TEXT("parameters"));
				}
				continue;
			}

			if (ToolObj->HasTypedField<EJson::Object>(TEXT("function")))
			{
				const TSharedPtr<FJsonObject> FunctionObj = ToolObj->GetObjectField(TEXT("function"));
				if (FunctionObj.IsValid() && FunctionObj->HasTypedField<EJson::Object>(TEXT("parameters")))
				{
					NormalizeSchemaObject(FunctionObj->GetObjectField(TEXT("parameters")), TEXT("parameters"));
				}
			}
		}
	}

	static FString GetToolNameForValidation(const TSharedPtr<FJsonObject>& ToolObj, bool bResponsesFormat)
	{
		if (!ToolObj.IsValid())
		{
			return TEXT("<invalid>");
		...
		if (bResponsesFormat)
		{
			return ToolObj->HasField(TEXT("name")) ? ToolObj->GetStringField(TEXT("name")) : TEXT("<missing name>");
		}

		if (ToolObj->HasTypedField<EJson::Object>(TEXT("function")))
		{
			const TSharedPtr<FJsonObject> FunctionObj = ToolObj->GetObjectField(TEXT("function"));
			if (FunctionObj.IsValid() && FunctionObj->HasField(TEXT("name")))
			{
				return FunctionObj->GetStringField(TEXT("name"));
			}
		}

		return TEXT("<missing function.name>");
	}

	static TArray<TSharedPtr<FJsonValue>> ConvertToolDefinitionsToResponses(
		const TArray<TSharedPtr<FJsonValue>>& ChatTools)
	{
		TArray<TSharedPtr<FJsonValue>> ResponseTools;
		ResponseTools.Reserve(ChatTools.Num());

		for (const TSharedPtr<FJsonValue>& ToolVal : ChatTools)
		{
			const TSharedPtr<FJsonObject> ChatTool = ToolVal.IsValid() ? ToolVal->AsObject() : nullptr;
			if (!ChatTool.IsValid() || !ChatTool->HasTypedField<EJson::Object>(TEXT("function")))
			{
				continue;
			}

			const TSharedPtr<FJsonObject> FunctionObj = ChatTool->GetObjectField(TEXT("function"));
			if (!FunctionObj.IsValid())
			{
				continue;
			}

			TSharedPtr<FJsonObject> ResponseTool = MakeShareable(new FJsonObject);
			ResponseTool->SetStringField(TEXT("type"), TEXT("function"));
			ResponseTool->SetStringField(TEXT("name"), FunctionObj->GetStringField(TEXT("name")));
			ResponseTool->SetStringField(TEXT("description"), FunctionObj->GetStringField(TEXT("description")));
			ResponseTool->SetObjectField(TEXT("parameters"), FunctionObj->GetObjectField(TEXT("parameters")));
			ResponseTools.Add(MakeShareable(new FJsonValueObject(ResponseTool)));
		}
...
TArray<TSharedPtr<FJsonValue>> FGitHubCopilotUEToolExecutor::BuildToolDefinitions(bool bResponsesFormat)
{
	TArray<TSharedPtr<FJsonValue>> Tools;
	...
	// ── spawn_actor ──
	{
		...
		AddProp(Props, TEXT("actor_class"), TEXT("string"), TEXT("Actor class to spawn (e.g. AActor, APointLight, AStaticMeshActor, ACharacter, or /Script/... path)"));
		AddProp(Props, TEXT("name"), TEXT("string"), TEXT("Optional display name for the spawned actor"));
		AddArrayProp(Props, TEXT("location"), TEXT("number"), TEXT("Spawn location [X, Y, Z] (default [0,0,0])"), 3, 3);
		AddArrayProp(Props, TEXT("rotation"), TEXT("number"), TEXT("Spawn rotation [Pitch, Yaw, Roll] in degrees (default [0,0,0])"), 3, 3);
		AddArrayProp(Props, TEXT("scale"), TEXT("number"), TEXT("Scale [X, Y, Z] (default [1,1,1])"), 3, 3);
		Params->SetObjectField(TEXT("properties"), Props);
		TArray<TSharedPtr<FJsonValue>> Req;
		Req.Add(MakeShareable(new FJsonValueString(TEXT("actor_class"))));
		Params->SetArrayField(TEXT("required"), Req);
	...
	// ── create_material_asset ──
	{
		...
		AddProp(Props, TEXT("asset_name"), TEXT("string"), TEXT("Material asset name (e.g. M_GoldMetal)"));
		AddProp(Props, TEXT("package_path"), TEXT("string"), TEXT("Content path (e.g. /Game/Materials). Default: /Game/Copilot/Materials"));
		AddArrayProp(Props, TEXT("base_color"), TEXT("number"), TEXT("Base color [R, G, B] values 0-1 (e.g. [0.8, 0.2, 0.1])"), 3, 3);
		AddProp(Props, TEXT("metallic"), TEXT("number"), TEXT("Metallic value 0-1 (default 0.0)"));
		AddProp(Props, TEXT("roughness"), TEXT("number"), TEXT("Roughness value 0-1 (default 0.5)"));
		AddArrayProp(Props, TEXT("emissive_color"), TEXT("number"), TEXT("Emissive color [R, G, B] (values can exceed 1 for glow)"), 3, 3);
		AddProp(Props, TEXT("opacity"), TEXT("number"), TEXT("Opacity 0-1 (setting this switches blend mode to Translucent)"));
		AddProp(Props, TEXT("blend_mode"), TEXT("string"), TEXT("Blend mode: opaque, translucent, additive, modulate, masked"));
		AddProp(Props, TEXT("shading_model"), TEXT("string"), TEXT("Shading model: default_lit, unlit, subsurface, clearcoat"));
		AddProp(Props, TEXT("assign_to"), TEXT("string"), TEXT("Optional: name of actor in level to assign material to"));
	...
	// ── set_actor_properties ──
	{
		...
		AddArrayProp(Props, TEXT("location"), TEXT("number"), TEXT("New location [X, Y, Z]"), 3, 3);
		AddArrayProp(Props, TEXT("rotation"), TEXT("number"), TEXT("New rotation [Pitch, Yaw, Roll] in degrees"), 3, 3);
		AddArrayProp(Props, TEXT("scale"), TEXT("number"), TEXT("New scale [X, Y, Z]"), 3, 3);
	...
	// ── delete_actors ──
	{
		...
		Params->SetStringField(TEXT("type"), TEXT("object"));
		TSharedPtr<FJsonObject> Props = MakeShareable(new FJsonObject);
		AddArrayProp(Props, TEXT("actor_names"), TEXT("string"), TEXT("Array of actor names or labels to delete"));
	...
	// ── retarget_animations ──
	{
		...
		AddArrayProp(Props, TEXT("animations"), TEXT("string"), TEXT("Specific animation paths to retarget. If omitted, retargets all animations using the source skeleton."));
	...
	// ── git_commit ──
	{
		...
		AddArrayProp(Props, TEXT("files"), TEXT("string"), TEXT("Specific files to stage. If omitted, stages all changes (git add -A)."));
	...
	NormalizeToolDefinitions(Tools, false);
	return bResponsesFormat ? ConvertToolDefinitionsToResponses(Tools) : Tools;
}

bool FGitHubCopilotUEToolExecutor::ValidateToolDefinitions(
	const TArray<TSharedPtr<FJsonValue>>& Tools,
	bool bResponsesFormat,
	TArray<FString>& OutErrors)
{
	...
	{
		const TSharedPtr<FJsonObject> ToolObj = Tools[ToolIndex].IsValid() ? Tools[ToolIndex]->AsObject() : nullptr;
		if (!ToolObj.IsValid())
		{
			OutErrors.Add(FString::Printf(TEXT("Tool[%d] is not a valid object"), ToolIndex));
			continue;
		}

		const FString ToolName = GetToolNameForValidation(ToolObj, bResponsesFormat);
		TSharedPtr<FJsonObject> Parameters;

		if (bResponsesFormat)
		{
			if (!ToolObj->HasField(TEXT("name")) || ToolObj->GetStringField(TEXT("name")).TrimStartAndEnd().IsEmpty())
			{
				OutErrors.Add(FString::Printf(TEXT("Responses tool[%d] is missing top-level name"), ToolIndex));
			}
			if (!ToolObj->HasTypedField<EJson::Object>(TEXT("parameters")))
			{
				OutErrors.Add(FString::Printf(TEXT("Responses tool '%s' is missing parameters object"), *ToolName));
				continue;
			}
			Parameters = ToolObj->GetObjectField(TEXT("parameters"));
		}
		else
		{
			if (!ToolObj->HasTypedField<EJson::Object>(TEXT("function")))
			{
				OutErrors.Add(FString::Printf(TEXT("Chat tool[%d] is missing function object"), ToolIndex));
				continue;
			}
			...
			if (!FunctionObj.IsValid())
			{
				OutErrors.Add(FString::Printf(TEXT("Chat tool[%d] has an invalid function object"), ToolIndex));
				continue;
			}
			if (!FunctionObj->HasField(TEXT("name")) || FunctionObj->GetStringField(TEXT("name")).TrimStartAndEnd().IsEmpty())
			{
				OutErrors.Add(FString::Printf(TEXT("Chat tool[%d] is missing function.name"), ToolIndex));
			}
			if (!FunctionObj->HasTypedField<EJson::Object>(TEXT("parameters")))
			{
				OutErrors.Add(FString::Printf(TEXT("Chat tool '%s' is missing function.parameters object"), *ToolName));
				continue;
			}
			Parameters = FunctionObj->GetObjectField(TEXT("parameters"));
		}

		if (!Parameters.IsValid())
		{
			OutErrors.Add(FString::Printf(TEXT("Tool '%s' has invalid parameters schema"), *ToolName));
			continue;
		}

		if (!Parameters->HasField(TEXT("type")) || Parameters->GetStringField(TEXT("type")) != TEXT("object"))
		{
			OutErrors.Add(FString::Printf(TEXT("Tool '%s' parameters.type must be 'object'"), *ToolName));
		}

		if (!Parameters->HasTypedField<EJson::Object>(TEXT("properties")))
		{
			continue;
		}

		const TSharedPtr<FJsonObject> Properties = Parameters->GetObjectField(TEXT("properties"));
		if (!Properties.IsValid())
		{
			continue;
		}
		...
		for (const TPair<FString, TSharedPtr<FJsonValue>>& PropPair : Properties->Values)
		{
			const TSharedPtr<FJsonObject> PropObj = PropPair.Value.IsValid() ? PropPair.Value->AsObject() : nullptr;
			if (!PropObj.IsValid())
			{
				OutErrors.Add(FString::Printf(TEXT("Tool '%s' property '%s' is not an object schema"), *ToolName, *PropPair.Key));
				continue;
			}

			if (!PropObj->HasField(TEXT("type")) || PropObj->GetStringField(TEXT("type")).TrimStartAndEnd().IsEmpty())
			{
				OutErrors.Add(FString::Printf(TEXT("Tool '%s' property '%s' is missing schema type"), *ToolName, *PropPair.Key));
				continue;
			}

			if (PropObj->GetStringField(TEXT("type")) == TEXT("array"))
			{
				if (!PropObj->HasTypedField<EJson::Object>(TEXT("items")))
				{
					OutErrors.Add(FString::Printf(TEXT("Tool '%s' array property '%s' is missing items schema"), *ToolName, *PropPair.Key));
					continue;
				}

				const TSharedPtr<FJsonObject> ItemsObj = PropObj->GetObjectField(TEXT("items"));
				if (!ItemsObj.IsValid() || !ItemsObj->HasField(TEXT("type")) || ItemsObj->GetStringField(TEXT("type")).TrimStartAndEnd().IsEmpty())
				{
					OutErrors.Add(FString::Printf(TEXT("Tool '%s' array property '%s' has invalid items.type"), *ToolName, *PropPair.Key));
				}
			}
		}
	}
