...
FCopilotResponse FGitHubCopilotUECommandRouter::HandleCreateBlueprintFunctionLibrary(const FCopilotRequest& Request)
{
	...
	if (ClassNameArg)
	{
		AssetName = *ClassNameArg;
	}
	else if (!Request.UserPrompt.IsEmpty())
	{
		AssetName = Request.UserPrompt;
		AssetName.ReplaceInline(TEXT(" "), TEXT(""));
	}

	if (AssetName.IsEmpty())
	{
		AssetName = TEXT("BP_CopilotFunctionLibrary");
	}

	if (!AssetName.StartsWith(TEXT("BP_")))
	{
		AssetName = TEXT("BP_") + AssetName;
	}

	FString PackagePath = TEXT("/Game/Copilot");
	const FString* PackagePathArg = Request.CommandArguments.Find(TEXT("PackagePath"));
	if (PackagePathArg && !PackagePathArg->IsEmpty())
	{
	...
	{
		Response.ResultStatus = ECopilotResultStatus::Failure;
		Response.ErrorMessage = FString::Printf(TEXT("Invalid package path '%s'. Use a valid Content path like /Game/MyFolder."), *PackagePath);
		return Response;
	}
	...
	{
		Response.ResultStatus = ECopilotResultStatus::Failure;
		Response.ErrorMessage = FString::Printf(TEXT("Asset already exists: %s"), *ObjectPath);
		return Response;
	}
	...
	{
		Response.ResultStatus = ECopilotResultStatus::Failure;
		Response.ErrorMessage = TEXT("Failed to create Blueprint Function Library asset");
		return Response;
	}
	...
	if (!UPackage::SavePackage(Package, NewBlueprint, *PackageFilename, SaveArgs))
	{
		...
		Response.ErrorMessage = FString::Printf(
			TEXT("Created Blueprint Function Library asset but failed to save package to disk: %s"),
			*PackageFilename);
		return Response;
	}
	...
	Response.ResponseText = FString::Printf(
		TEXT("Created and saved Blueprint Function Library asset:\n  Name: %s\n  Package: %s\n  Object Path: %s\n  File: %s"),
		*AssetName, *PackageName, *ObjectPath, *PackageFilename);
